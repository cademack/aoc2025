...
int main()
{
    ...
    size_t entry_index = lines[0].find('S');
    std::cout << "Start Idx: " << entry_index << std::endl;
    ...
    for(size_t line_i = 1; line_i < num_lines; line_i++)
    {
        ...
        std::set<size_t> new_lasers;
        std::cout << "line : " << line_i << std::endl;
        // std::cout << "content : " << cur_line << std::endl;
        ...
        for(size_t char_i = 0; char_i < line_length; char_i++)
        {
            ...
            if(cur_char == '.' && (above_char == '|' || above_char == '|'))
            {
                cur_line[char_i] = '|';
            }
        }
        ...
        std::cout << "Adding: " << new_lasers.size() << std::endl;
        std::cout << "=====================" << std::endl;
        lines[line_i] = cur_line;
        total_sum += new_lasers.size();
    }
